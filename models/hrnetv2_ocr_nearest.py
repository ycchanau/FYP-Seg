...
class HRNetV2_OCR_Nearest(BaseModel):
    def __init__(self,  num_classes, pretrained=True, **kwargs):
        super(HRNetV2_OCR_Nearest, self).__init__()
        extra = {
    ...
    def forward(self, x, return_feature_maps=False):
        ...
        x1 = F.interpolate(
            x[1], size=(x0_h, x0_w), mode='nearest')
        x2 = F.interpolate(
            x[2], size=(x0_h, x0_w), mode='nearest')
        x3 = F.interpolate(
            x[3], size=(x0_h, x0_w), mode='nearest')
        ...
        out = F.interpolate(
            out, size=size, mode='bilinear', align_corners=ALIGN_CORNERS)
        out_aux = F.interpolate(
            out_aux, size=size, mode='bilinear', align_corners=ALIGN_CORNERS)
