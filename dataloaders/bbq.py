...
class BBQStuff(BaseDataSet):
    def __init__(self, warp_image = True, **kwargs):
        self.warp_image = warp_image
        self.num_classes = 4
        self.palette = palette.COCO_palette
        super(BBQStuff, self).__init__(**kwargs)

    def _set_files(self):
        self.image_dir = os.path.join(self.root, "r_photo")
        self.label_dir = os.path.join(self.root, "r_mask")
        self.index_dir = os.path.join(self.root, "r_index")
...
class BBQ(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=False, num_workers=1,
        ...
        self.dataset = BBQStuff(**kwargs)
        super(BBQ, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)