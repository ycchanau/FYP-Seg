...
def batch_class_pixel_accuracy(output, target, cls):
    output = output.data
    _, predict = torch.max(output, 1)
    predict = predict.int()
    target = target.int()
    pixel_labeled = (target == cls).sum()
    pixel_correct = ((predict == target)*(target == cls)).sum()
    return pixel_correct.cpu().numpy(), pixel_labeled.cpu().numpy()


class Trainer(BaseTrainer):
    def __init__(self, model, loss, resume, fine_tune, config, train_loader, val_loader=None, train_logger=None, prefetch=True):
        super(Trainer, self).__init__(model, loss, resume, fine_tune, config, train_loader, val_loader, train_logger)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            if self.config['arch']['type'][:3] == 'PSP':
                ...
                output = output[0]
            elif self.config['arch']['type'][:2] == 'IC':
                assert output[0].size()[2:] == target.size()[1:]
                assert output[0].size()[1] == self.num_classes 
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(tbar):
                ...
                for i in range(self.num_classes):
                    cls_pix_correct, cls_pix_labeled=batch_class_pixel_accuracy(output,target,i)
                ...
                # PRINT INFO
                pixAcc, mIoU, _ = self._get_seg_metrics().values()
                cls_pix_acc=np.round(cls_total_pix_correct/cls_total_pix_labeled,3)
                tbar.set_description('EVAL ({}) | Loss: {:.3f}, PixelAcc: {:.2f}, Mean IoU: {:.2f}, cls_pix_acc: {} |'.format( epoch,
                                                self.total_loss.average,
                                                pixAcc, mIoU, str(cls_pix_acc)))
