...
class BaseDataSet(Dataset):
    def __init__(self, root, split, mean, std, base_size=None, augment=False, val=False,
                crop_size=321, scale=False, flip=False, rotate=False, blur=False, return_id=False):
        self.root = root
        self.split = split
        self.mean = mean
        self.std = std
    ...
    def _val_augmentation(self, image, label):
        if self.base_size:
            image = cv2.resize(image, (self.base_size, self.base_size), interpolation=cv2.INTER_LINEAR)
            label = cv2.resize(label, (self.base_size, self.base_size), interpolation=cv2.INTER_NEAREST)
    ...
    def __getitem__(self, index):
        ...
        #print("unique value: {}".format(np.unique(image)))
