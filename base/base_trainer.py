...
class BaseTrainer:
    def __init__(self, model, loss, resume, fine_tune, config, train_loader, val_loader=None, train_logger=None):
        self.model = model
        self.loss = loss
        self.config = config
        ...
        if fine_tune: self._start_finetune(fine_tune)
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs+1):
            ...
            if self.mnt_mode != 'off' and epoch % self.config['trainer']['val_per_epochs'] == 0:
                print("mode: {} mnt_metric: {} mnt_best: {} mnt_now: {}".format(self.mnt_mode, self.mnt_metric, self.mnt_best, log[self.mnt_metric]))
                try:
        ...
        self.checkpoint_dir='/home/ubuntu/pytorch_segmentation-master/'+self.checkpoint_dir
        cfg_pth=os.path.join(self.checkpoint_dir, 'config.json')
        model_pth=os.path.join(self.checkpoint_dir, 'best_model.pth')
        img_folders=['/home/ubuntu/BBQ/r_train','/home/ubuntu/BBQ/r_trainval','/home/ubuntu/BBQ/r_val']
        for img_folder in img_folders:
            os.system("python inference.py --config {} --model {} --image {}".format(cfg_pth,model_pth,img_folder))
    ...
    def _start_finetune(self, resume_path):
        self.logger.info(f'Loading checkpoint : {resume_path}')
        checkpoint = torch.load(resume_path)
        ...
        if checkpoint['config']['loss'] != self.config['loss']:
            self.logger.info(f"Replace {checkpoint['config']['loss']} with {self.config['loss']} for fine_tune")
        self.optimizer.load_state_dict(checkpoint['optimizer'])
        # if self.lr_scheduler:
        #     self.lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
